...
function getInstanceType(node, originalStr, replacedPropValues = []) {
    ...
    const propsStr = getNameValuePairsFromAttributes(node, originalStr)
        ...
        .join(', ');
    return surroundWithIgnoreComments(`new ${node.name}({target: __sveltets_any(''), props: {${propsStr}}})`);
}
/**
...
function getInstanceTypeForDefaultSlot(node, originalStr, replacedPropsPrefix) {
    ...
    const propsStr = getNameValuePairsFromAttributes(node, originalStr)
        ...
        .join(', ');
    const str = surroundWithIgnoreComments(`new ${node.name}({target: __sveltets_any(''), props: {${propsStr}}})`);
        ...
        if (attr.value.length === 1) {
            ...
            if (val.type === 'MustacheTag') {
                ...
                if (val.expression.type === 'Literal') {
                    const value = typeof val.expression.value === 'string' ? '""' : val.expression.value;
                    return { name, value };
                }
...
// prettier-ignore
const svelteShims = `declare class Svelte2TsxComponent<
    Props extends {} = {},
    Events extends {} = {},
    Slots extends {} = {}
> {
...
function addComponentExport({ str, uses$$propsOr$$restProps, strictEvents, isTsFile, getters, usesAccessors, exportedNames, fileName, componentDocumentation, mode }) {
